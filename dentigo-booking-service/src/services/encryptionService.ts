...
export async function verifyDentist(
    text: string,
    signature: string,
): Promise<boolean> {
    const publicKey = await downloadKey('clinicServiceSign')
    return verify(text, signature, publicKey)
}
...
export async function verifyPatient(
    ...
    const publicKey = await downloadKey('userServiceSign')
    return verify(text, signature, publicKey)
}
